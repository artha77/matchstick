...
** Last update Fri Feb 24 09:25:35 2017 dylan renard
*/
...
t_ld		*rot_list(t_ld *list)
{
  ...
  if (list != NULL && list->next != list->end)
    {
      val = list->next->number;
      list->next->number = list->end->number;
      list->end->number = val;
      return (list);
    }
  return (list);
  ...
  if (list->next != NULL && list->end != NULL)
    {
      ...
      (list->next)->number = ((list->next)->next)->number;
      ((list->next)->next)->number = tmp;
      return (list);
    }
  return (list);
}
